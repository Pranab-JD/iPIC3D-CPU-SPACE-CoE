...
with h5py.File(hdf_files[0], "r") as f:
    sample = np.array(f["moments/rho/" + time_cycle])
...
for rank in range(num_expected_files):
    ...
    with h5py.File(file_path, "r") as f:
        ...
        rho_data = np.array(f["moments/rho/" + time_cycle])
        ...
        rho[x0:x0 + nx_local, y0:y0 + ny_local] = rho_data[:, :, 0]

###? Plot (2D)
fig = plt.figure(figsize = (8, 6), dpi = 150)

plt.imshow(rho, origin='lower', cmap='seismic', aspect = "auto")
...
plt.savefig(dir_data + "Density_" + time_cycle + ".eps")
plt.close()
