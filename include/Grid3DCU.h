...
public:
    ...
    Grid3DCU(int nxc_, int nyc_, int nzc_, double dx_, double dy_, double dz_,
    ...
    void gradC2N(arr3_double gradXN, arr3_double gradYN, arr3_double gradZN, const_arr3_double scFieldC)const;
    
    //* calculate gradient on nodes, given a scalar field defined on central points *//
    void gradN2C(arr3_double gradXC, arr3_double gradYC, arr3_double gradZC, const_arr3_double scFieldN)const;
    
    //* calculate divergence on central points, given a vector field defined on nodes *//
    void divN2C(arr3_double divC, const_arr3_double vecFieldXN, const_arr3_double vecFieldYN, const_arr3_double vecFieldZN)const;
    
    //* calculate divergence on nodes, given a vector field defined on central points *//
    void divC2N(arr3_double divN, const_arr3_double vecFieldXC, const_arr3_double vecFieldYC, const_arr3_double vecFieldZC)const;
    
    //* calculate curl on nodes, given a vector field defined on central points *//
    void curlC2N(arr3_double curlXN, arr3_double curlYN, arr3_double curlZN, 
                const_arr3_double vecFieldXC, const_arr3_double vecFieldYC, const_arr3_double vecFieldZC)const;
    ...
    void divSymmTensorN2C(arr3_double divCX, arr3_double divCY, arr3_double divCZ,
                        const_arr4_double pXX, const_arr4_double pXY, const_arr4_double pXZ, 
                        const_arr4_double pYY, const_arr4_double pYZ, const_arr4_double pZZ, int ns)const;
    ...
    void divBCleft(arr3_double divBC, const_arr3_double vectorX, const_arr3_double vectorY, const_arr3_double vectorZ, int leftActiveNode, int dirDER)const;
    
    //* calculate divergence on boundaries *//
    void divBCright(arr3_double divBC, const_arr3_double vectorX, const_arr3_double vectorY, const_arr3_double vectorZ, int rightActiveNode, int dirDER)const;
    
    //* calculate derivative on boundaries *//
    void derBC(arr3_double derBC, const_arr3_double vector, int leftActiveNode, int dirDER)const;
...
private:
    ...
    //* Grid conventions:
    //
    //   [The purpose of this note is not so much to teach as to establish a consistent vocabulary and set of naming conventions for grid parameters.]
    //
    //   The purpose of iPic3D is to solve a Vlasov-Maxwell plasma problem. Each problem is solved on a box-shaped domain and is defined by initial 
    //   conditions and boundary conditions.
    //
    //   Conceptually, the full problem domain is solved on a "guarded" global grid. The guarded global grid consists of proper grid cells (that partition the domain)
    //   surrounded by a layer of "guard" or "ghost" cells that are used to handle boundary conditions. At the beginning of each cycle of the algorithm, the guard 
    //   cells are populated with data based on the choice of boundary conditions of the problem. Refer to the layer of proper cells just inside the guard cells
    //   as "(proper) boundary cells". Guard cell data is generally populated with data from boundary cells. For periodic boundary conditions, guard cell data is
    //   simply copied from the appropriate boundary cell. For conducting wall and outflow boundaries, each guard cell is populated based on the data from the
    //   neighboring boundary cell, probably with reflection of location and often with negation of some values, depending on the type of boundaries involved.
    //
    //   The corners of the mesh cells are referred to as "nodes".  In iPic3D, electromagnetic field quantities are canonically represented as sample values at 
    //   the nodes, and the value of the electromagnetic field in the interior of a grid cell is determined by bilinear interpolation of the field values at the
    //   nodes.  Bilinear interpolation is also used when converting between a representation as cell-average quantities and a representation as nodal smaples.
    //   This maintains second-order accuracy in space.
    //
    //   To parallelize with MPI, the full domain of the problem is partitioned into "(proper) subdomains", one for each process, such that the global grid partitions the
    //   subdomains.  Refer to the grid cells within a subdomain as its "(proper) subgrid". Refer to the outer layer of cells within each subgrid as "boundary cells".  
    //   The grid defined in each process is a "guarded subgrid", which consists of the proper subgrid surrounded by a layer of guard cells corresponding either to the 
    //   boundary cells of the neighboring subdomain or to ghost cells of the guarded global grid.
    //
    //   The guard cells thus lie outside the proper physical subdomain for which the process is responsible. Guard cells are used to handle boundary conditions and
    //   interprocess communication.  At the beginning of each cycle of the algorithm, the guard cells are repopulated. Guard cells outside the problem domain are 
    //   populated using boundary conditions. Any guard cell inside the problem domain is populated from the appropriate boundary cell of the appropriate process.
    //
...
private:
    ...
    /** volume of mesh cell*/
    double VOL;
    //** invol = inverse of volume (same for both computed on cell centres and nodes) *//
    double invVOL;
...
public: 
    ...
    // coordinate accessors: calculated equivalents (preferred for accelerator?):
